...
class MsgAnalysisAgent:
    def __init__(self, api_key: str):
        self.wrapper = GoogleWrapper(api_key=api_key)

    def judge_relatedness(self, msg1: str, msg2: str) -> bool:
        prompt = dedent(
            f"""
        <Role>
        ...
        response = self.wrapper.generate(prompt=prompt, structure=Relatedness)

        return response.related

    def analyze_message(self, message: str) -> MsgMetadata:
        prompt = dedent(
            f"""
        <Role>
        You are a professional text analyst.
        ...
        response = self.wrapper.generate(prompt=prompt, structure=MsgMetadata)

        return response

    def summarize_thread(self, messages: list) -> ThreadMetadata:
        prompt = dedent(
            f"""
        <Role>
        You are a professional text analyst.
        ...
        response = self.wrapper.generate(prompt=prompt, structure=ThreadMetadata)
